angular.module('bbsDire').directive('confirmDirective',['domService','utilService',function(domService,utilService){
    ...
    return {
        ...
        link: function(scope,element,attr){
            ...
            utilService.fetch(scope,scope.popupObj,defaultConfig,scope.popupObj);
            var popupPanelHtml = '<div style="width:'+ scope.popupObj.width +'px;z-index:100;border-radius:4px;margin:0px;overflow: hidden;position: relative;"></div>';
            ...
            element.on('click',function(){
                ...
                popupPanelEle.css({position: 'absolute',top : (pos.top - popupEleHeight) + 'px',left : pos.left - Math.abs(scope.popupObj.width - elementWidth)/2 + 'px'});
                popupBodyEle.remove();
                ...
                popupBodyEle.find('.popup-ok').on('click',function(){
                    popupBodyEle.animate({top : popupEleHeight + 'px'},'fast',null,function(){
                        removeEle();
                        scope.popupObj.okFn({element : element,attr : attr});
                    });
                });
                ...
                popupBodyEle.find('.popup-cancel').on('click',function(){
                    popupBodyEle.animate({top : popupEleHeight + 'px'},'fast',null,function(){
                        removeEle();
                        scope.popupObj.cancelFn({element : element,attr : attr});
                    });
                });
